...
const Favicon = ({ size, source }: { size: number; source: AISource }) => (
  <span
    className="flex items-center justify-center overflow-hidden rounded-full border border-hairline bg-surface text-muted-foreground"
        ...
        {isOpen ? (
          ...
          >
            {sources.map((source, index) => (
              ...
              >
                <a
                  className="flex items-start gap-2.5 rounded-lg px-1 py-1.5 no-underline transition-colors hover:bg-canvas-subtle"
                  href={source.url}
